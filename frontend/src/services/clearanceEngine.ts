...
import { CardSample } from '../types/ocr';
import { RiskEvaluation } from '../types/risk';
import { DenialCategoryStat, PayerDenialStat, PreventiveRule } from '../types/analytics';

class ClearanceStore {
  private queue: ClearanceItem[] = [...mockClearanceItems];
  private dossiers: Record<string, PatientDossier> = { ...mockDossiers };
  private riskEvaluations: Record<string, RiskEvaluation> = { ...mockRiskEvaluations };
  private cardSamples: CardSample[] = [...mockCardSamples];
  private denialCategories: DenialCategoryStat[] = [...mockDenialCategories];
  private payerStats: PayerDenialStat[] = [...mockPayerStats];
  private rules: PreventiveRule[] = [...mockPreventiveRules];
  private activePatientId: string = 'PAT-1082';

  public getQueue(): ClearanceItem[] {
    return this.queue;
  }

  public getPatientDossier(patientId: string): PatientDossier | undefined {
    return this.dossiers[patientId] || this.createFallbackDossier(patientId);
  }

  public getRiskEvaluation(patientId: string): RiskEvaluation | undefined {
    return this.riskEvaluations[patientId] || this.createFallbackRisk(patientId);
  }

  public getCardSamples(): CardSample[] {
    return this.cardSamples;
  }

  public getDenialCategories(): DenialCategoryStat[] {
    return this.denialCategories;
  }

  public getPayerStats(): PayerDenialStat[] {
    return this.payerStats;
  }

  public getRules(): PreventiveRule[] {
    return this.rules;
  }
  ...
  public approvePriorAuth(patientId: string, authNumber: string = 'AUTH-PA-2026-X99'): void {
  ...
  public syncCardOcr(sampleId: string): void {
  ...
  private createFallbackDossier(patientId: string): PatientDossier {
  ...
  private createFallbackRisk(patientId: string): RiskEvaluation {
    const item = this.queue.find(q => q.patient_id === patientId);
    return {
      patient_id: patientId,
